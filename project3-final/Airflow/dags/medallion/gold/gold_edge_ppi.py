...
GOLD_EDGE_PPI_CSV = "gold_edge_ppi.csv"
GOLD_EDGE_PPI_GEO_CSV = "gold_edge_ppi_geo.csv"
...
def _normalize_edge_row(
    row: dict[str, Any],
    string_id_to_geneid: dict[str, str],
) -> dict[str, Any]:
    node_a = str(row["node1_string_id"])
    node_b = str(row["node2_string_id"])
    if node_a <= node_b:
        canonical_node1, canonical_node2 = node_a, node_b
        normalized = dict(row)
    else:
        canonical_node1, canonical_node2 = node_b, node_a
        normalized = {
            "geneid": row.get("geneid", ""),
            "node1": row["node2"],
            "node2": row["node1"],
            "node1_string_id": node_b,
            "node2_string_id": node_a,
            "experimentally_determined_interaction": row["experimentally_determined_interaction"],
            "database_annotated": row["database_annotated"],
            "automated_textmining": row["automated_textmining"],
            "combined_score": row["combined_score"],
        }

    if canonical_node1 in string_id_to_geneid:
        normalized["geneid"] = string_id_to_geneid[canonical_node1]
    elif canonical_node2 in string_id_to_geneid:
        normalized["geneid"] = string_id_to_geneid[canonical_node2]
    return normalized
...
def _deduplicate_rows(
    ...
    for row in rows:
        ...
        normalized = _normalize_edge_row(row, string_id_to_geneid)
        if edge_key not in dedup_map:
            dedup_map[edge_key] = normalized
            continue
        current_score = _safe_float(dedup_map[edge_key].get("combined_score"))
        candidate_score = _safe_float(normalized.get("combined_score"))
...
def _run_gold_edge_ppi_from_silver(silver_csv: str, output_csv: str) -> None:
    """Gera arestas PPI via STRING API a partir de um CSV silver com ``geneid`` e ``symbol``."""
    silver_path = data_dir() / "silver" / silver_csv
    output_path = data_dir() / "gold" / output_csv
    ...
    LOGGER.info(
        "PPI Gold (%s) iniciado: input=%s genes de entrada (unicos por geneid)=%s",
        output_csv,
        silver_csv,
        len(genes),
    )
    ...
    dedup_rows.sort(
        key=lambda row: (
            str(row["node1_string_id"]),
            str(row["node2_string_id"]),
        )
    )
...
def run_gold_edge_ppi(**_context) -> None:
    ...
    _run_gold_edge_ppi_from_silver(SILVER_GEO_NODES_PRINCIPAL_CSV, GOLD_EDGE_PPI_CSV)


def run_gold_edge_ppi_geo(**_context) -> None:
    """Gera ``gold_edge_ppi_geo.csv`` a partir de ``silver_geo_nodes.csv``."""
    _run_gold_edge_ppi_from_silver(SILVER_GEO_NODES_CSV, GOLD_EDGE_PPI_GEO_CSV)
