...
│       └── gold/              # gold.py, gold_edge_ppi.py, gold_geo_nodes.ipynb
├── data/
│   └── raw/                   # entrada versionada
│       ├── geo/               # GSE251849, GSE275334, NOS0001 (.csv, .soft.gz)
│       └── nih/               # NIH.csv
├── pathwayApp/                # visualização KEGG map05171 (HTML/JS)
...
| `data/raw/geo/`, `data/raw/nih/` | Fontes brutas GEO e metadados clínicos NIH. |
| `data/bronze/`, `silver/`, `gold/` | Saídas geradas na execução (criadas automaticamente). |
| `pathwayApp/` | Scripts e HTML de correlação gene-pathway (KEGG map05171). |
...
Documentação do projeto, slides e modelo lógico: [`../README.md`](../README.md), [`../assets/`](../assets/). Análise de rede no Cytoscape: [`../Cytoscape/`](../Cytoscape/).
...
**2.** Entre na **raiz deste módulo Airflow** (ajuste `<usuario>` se precisar):

```bash
cd "/mnt/c/Users/<usuario>/Documents/ProjectsMO430/project3-final/Airflow"
```
...
Se o repo estiver só em disco Linux (ex.: `~/ProjectsMO430/project3-final/Airflow`), rode o `bash scripts/bootstrap-wsl-venv.sh` na mesma raiz; para venv dentro do repo, use `export MO430_VENV="$PWD/.venv"` antes do `bash scripts/bootstrap-wsl-venv.sh`.

## Uso

**1.** Na raiz de `project3-final/Airflow/`, carregue variáveis e venv (Bash):
...
Fluxo (resumo): `bronze_geo_soft_ingest` → `silver_geo_nodes` e `silver_geo_nodes_principal` → `gold_geo_nodes`; `silver_geo_nodes` → `gold_edge_ppi_geo`; `silver_geo_nodes_principal` → `gold_edge_ppi`; `bronze_ebi_gxa_ingest` → `silver_ebi_nodes`.
...
|-------|------------------|
| Bronze | `data/bronze/bronze_*.csv` (GEO CSV em `data/raw/geo/`, EBI, …) |
| Silver | `data/silver/silver_geo_nodes.csv`, `silver_geo_nodes_principal.csv`, `silver_ebi_nodes.csv`, … |
| Gold | `data/gold/gold_geo_nodes.csv`, `gold_edge_ppi.csv`, `gold_edge_ppi_geo.csv` |
...
| Bronze EBI | `dags/medallion/bronze/bronze_ebi.py` | API EBI → `data/bronze/bronze_ebi_expression.csv`. |
...
| Gold PPI | `dags/medallion/gold/gold_edge_ppi.py` | STRING-DB → `gold_edge_ppi.csv`, `gold_edge_ppi_geo.csv`. |
...
- Entradas em `data/raw/geo/` e `data/raw/nih/` são versionadas; `data/bronze/`, `data/silver/` e `data/gold/` são artefatos locais de execução.
